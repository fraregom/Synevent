...
import com.rilixtech.agendacalendarview.agenda.AgendaListView;
...
public class ListViewScrollTracker {
  ...
  private SparseIntArray mListViewItemHeights = new SparseIntArray();
  ...
  /**
   * Call from an AbsListView.OnScrollListener to calculnullate the scrollY (Here
   * we definite as the distance in pixels compared to the position representing the current
   * date).
   *
   * @param firstVisiblePosition First visible item position in the list.
   * @param visibleItemCount Number of visible items in the list.
   * @return Distance in pixels compared to current day position (negative if firstVisiblePosition less than
   * mReferencePosition)
   */
    ...
    if (visibleItemCount > 0) {
      ...
      if (mFirstVisiblePosition >= mReferencePosition) {
        for (int i = mReferencePosition; i < firstVisiblePosition; ++i) {
          if (mListViewItemHeights.get(i) == 0) {
